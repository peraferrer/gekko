...
export default function(_data, _trades) {
    ...
    var trades = _trades.map(t => {
        t.start = t.date
        t.date = toDate(t.date)
        const candle = _.find(data, d => { 
            const diff = moment.utc(t.start*1000).diff(moment.utc(d.start*1000), 'minutes');
            return  diff <= 0
        })
        t.type = t.action
         if (candle === undefined) {
            t.candle = {low: t.price, high: t.price}
         } else {
            t.candle = candle
         }
        return t
    ...
    svg.select("g.sma.ma-0").datum(techan.indicator.ema().period(12)(data)).call(ema0);
    svg.select("g.sma.ma-1").datum(techan.indicator.ema().period(26)(data)).call(ema1);
    ...
    function draw() {
        ...
        svg.select("g .sma.ma-0").call(ema0.refresh);
        svg.select("g .sma.ma-1").call(ema1.refresh);
    ...
    function refreshText(d) {
        valueText.text( d.id.toUpperCase() + " - " + d.type.toUpperCase() + ": " + moment.unix(d.start).utc().format("DD-MM-YYYY HH:mm") + " - " + valueFormat(d.price));
    }
